...
function findGoodEndings(goodEndings, badEndings, choices) {
    ...
    let stack = [1];
    let length = goodEndings[goodEndings.length - 1] > badEndings[badEndings.length - 1] ? goodEndings[goodEndings.length - 1] : badEndings[badEndings.length - 1]
    let result = [];
    let visited = {};
    while (stack.length) {
        ...
        if (goodEndingsMap.has(current)) {
            result.push(current);
        }
        else if (badEndingsMap.has(current)) {
            continue;
        }
        else if (choicesMap.has(current)) {
            let choice = choicesMap.get(current);
            stack.push(choice.option1);
            stack.push(choice.option2);
        }
