...
function shopping(products, list) {
    ...
    for (let product of products) {
        productsMap[product[0]] = product[1];
    }
    let normalVisitCounts = 0, currentDepartment = "";
    for (let item of list) {
        if (currentDepartment !== productsMap[item]) {
            currentDepartment = productsMap[item];
            normalVisitCounts += 1
        }
    }
    let listMap = new Map();
    for (let item of list) {
        if (!listMap.has(item)) {
            listMap.set(productsMap[item], true);
        }

    }
    return normalVisitCounts - listMap.size;
}
products = [
    ["Cheese", "Dairy"],
    ["Carrots", "Produce"],
