...
can move to in a single turn, given the following inputs:
...
function destinations(teleporter, die, startAtBoard, boardEnd) {
    let telMap = {};
    ...
    let visited = {};
    while (i <= die) {
        let nextPos = startAtBoard + i;
        //break out is nextpos is end
        if (nextPos === boardEnd) {
            result.push(nextPos);
            break;
        }
